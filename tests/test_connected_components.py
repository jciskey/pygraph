...
class ConnectedComponentsTest(unittest.TestCase):
	...
	def test_correct_connected_components(self):
		...
		for kc in known_components:
			...
			for c in components:
				#--Determine if the current component is a superset of known component (it might have more nodes than the known component)
				superset_match = True
				for n in kc:
					if n not in c:
						...
						break
				if superset_match:
					#--Determine if the current component has precisely the same number of nodes in it as the known component
					found_known_component = (len(kc) == len(c))
				if found_known_component:
					found_components_count += 1
					break
			if not found_known_component:
				#--We know the current component was not found in the connected components list, fail with an error message
		...
		#--Prior tests should stop things before we get this far if there are errors, but it's a simple sanity check test
	...
	def test_empty_cc_as_subgraphs(self):
		"""Does the ''get_connected_components_as_subgraphs'' function return an empty list when given an empty graph?"""
		graph = UndirectedGraph()
		...
		for kc in known_components:
			...
			for c in components:
				...
				if node_superset_match and edge_superset_match:
					#--Determine if the current component has precisely the same number of nodes and edges as the known component
					found_known_component = ((len(kc.get_all_node_ids()) == len(c.get_all_node_ids())) and (len(kc.get_all_edge_ids()) == len(c.get_all_edge_ids())))
				if found_known_component:
					found_components_count += 1
					break
			if not found_known_component:
				#--We know the current component was not found in the connected components list, fail with an error message
				component_string = '[{}]'.format(','.join(['(Nodes: {}, Edges: {})'.format(c.get_all_node_ids(), c.get_all_edge_ids()) for c in components]))
				msg = 'Component (Nodes: {}, Edges: {}) not found in {}'.format(kc.get_all_node_ids(), kc.get_all_edge_ids(), component_string)
				self.fail(msg)
